...
const FeedbackList = ({ parkingZoneId }) => {
    ...
    const feedbackServie = feedBackServices();
    useEffect(() => {
        feedbackServie
            .getByParkingZone(parkingZoneId, queryParam.page, queryParam.pageSize)
            .then(res => {
    ...
    const getFeedbackItem = (item, index) => {
        return (
            <List.Item key={index}>
                <List.Item.Meta
                    title={<><Rate value={item.rate} />  {dayjs(item.createdAt).format('YYYY-MM-DD HH:mm:ss')}</>}
