...
namespace Service.ManagerVPS.ExternalClients
{
    public class FileManagementClient
    {
        ...
        const string OBJECT_GET_ONE_URI = "api/{0}/object/get-one?objectName={1}&eTag={2}&versionId={3}&download={4}";
        const string OBJECT_DELETE_MULTIPLE = "api/{0}/object/multiple";
        const string OBJECT_EXIST_URI = "api/{0}/object/exist?objectName={1}&eTag={2}&versionId={3}";
        ...
        public async Task<List<ListObjectsDto>> GetObjects(string bucket, string prefix = null, bool recursive = false)
        {
        ...
        public async Task Upload(string bucket, string folderPath, byte[] fileBytes, string fileName)
        {
        ...
        public async Task Upload(string bucket, string folderPath, MultipartFormDataContent multipartFormDataContent)
        {
            string uri = $"{string.Format(OBJECT_URI, bucket)}?folderPath={folderPath}";
            HttpResponseMessage httpResponseMessage = await restClient.PostAsync(uri, multipartFormDataContent);
        ...
        public async Task<(byte[] respones, MediaTypeHeaderValue contentType)> Download(string bucket,
            string objectName,
            string eTag = "",
            string versionId = "")
        {
        ...
        async Task<HttpResponseMessage> GetOne(string bucket,
            ...
            bool download = false)
        {
            string uri = $"{string.Format(OBJECT_GET_ONE_URI, bucket, objectName, eTag, versionId, download)}";
